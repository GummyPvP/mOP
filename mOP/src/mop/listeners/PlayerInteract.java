...
public class PlayerInteract implements Listener {
	...
	public void onVoucherUse(PlayerInteractEvent e) {
		...
		if (p.getItemInHand().getItemMeta().getDisplayName().equals(ChatColor.translateAlternateColorCodes('&', "&a&lSlime Kit &7&o(Right click to redeem)"))) {
			...
			return;
		} else 	if (p.getItemInHand().getItemMeta().getDisplayName().equals(ChatColor.translateAlternateColorCodes('&', "&a&lKeys &7&o(Right click to redeem)"))) {
		...
		} else 	if (p.getItemInHand().getItemMeta().getDisplayName().equals(ChatColor.translateAlternateColorCodes('&', "&c&lHaribo Kit &7&o(Right click to redeem)"))) {
			...
			Bukkit.getServer().dispatchCommand(Bukkit.getServer().getConsoleSender(), "gkit " + p.getName() + " haribo");
			return;
			
		} else 	if (p.getItemInHand().getItemMeta().getDisplayName().equals(ChatColor.translateAlternateColorCodes('&', "&4&lGummy Kit &7&o(Right click to redeem)"))) {
		...
		} else 	if (p.getItemInHand().getItemMeta().getDisplayName().equals(ChatColor.translateAlternateColorCodes('&', "&e&lRandom &7&o(Right click to redeem)"))) {
			...
			if (r.nextInt(5) == 0) {
				...
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',
						"&8&l» &b" + p.getName()+ " has won 100K!"));
			} else if (r.nextInt(5) == 2) {
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',
						"&8&l» &b" + p.getName()+ " has won a money party for all online players!"));
				for (Player online : Bukkit.getOnlinePlayers()) {
					online.playSound(p.getLocation(), Sound.FIREWORK_LAUNCH,
							1F, 1F);
					online.playSound(p.getLocation(),
							Sound.FIREWORK_LARGE_BLAST2, 1F, 1F);
					mEconAPI.addMoney(online, 500000);
					online.sendMessage(ChatColor
				...
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',
						"&8&l» &b" + p.getName()+ " has won a rare key!"));
				Manager.getInstance().giveKey(p, KeyType.RARE);
				
			} else if (r.nextInt(5) == 4) {
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',
						"&8&l» &b" + p.getName()+ " has won 100 mcMMO credits!"));
				Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "addcredits "
						+ p.getName() + " 100");
		} else {
			Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',
		...
		} else 	if (p.getItemInHand().getItemMeta().getDisplayName().equals(ChatColor.translateAlternateColorCodes('&', "&a&lSloom Kit &7&o(Right click to redeem)"))) {
	...
	public void onInteract(PlayerInteractEvent e) {
		...
		if (p.getItemInHand().getItemMeta().getDisplayName()
				.equals(ChatColor.GREEN + "Common Crate Key")) {
			e.setCancelled(true);
			Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',
					"&8&l» &b" + p.getName()
							+ " is opening a &aCommon Crate&b..."));
			p.playEffect(e.getClickedBlock().getLocation(), Effect.STEP_SOUND,
					5);
			...
			if (r.nextInt(3) == 0) {
				...
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes(
						'&', "&8&l» &b" + p.getName()
								+ " won... &atheir very own pet kitty!"));

				Ocelot cat = (Ocelot) p.getWorld().spawnEntity(p.getLocation(),
						EntityType.OCELOT);
				cat.setCustomNameVisible(true);
				cat.setCustomName(ChatColor.BLUE + p.getName() + "'s "
						+ ChatColor.AQUA + "pet kitty");
				cat.setTamed(true);
				cat.setOwner(p);
				cat.setBaby();
				cat.setAgeLock(true);
		...
		if (p.getItemInHand().getItemMeta().getDisplayName()
				.equals(ChatColor.GOLD + "Uncommon Crate Key")) {
			e.setCancelled(true);
			Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes('&',
					"&8&l» &b" + p.getName()
							+ " is opening a &6Uncommon Crate&b..."));
			p.playEffect(e.getClickedBlock().getLocation(), Effect.STEP_SOUND,
					5);
			if (p.getItemInHand().getAmount() == 1)
				p.getInventory().removeItem(p.getItemInHand());
			...
			if (r.nextInt(5) == 0) {
				...
				if (r.nextInt(1) == 0) {
					Bukkit.broadcastMessage(ChatColor
							.translateAlternateColorCodes('&',
									"&8&l» &b" + p.getName()
											+ " won... &ca Rare Crate Key!"));
					Manager.getInstance().giveKey(p, KeyType.RARE);
				} else if (random.nextInt(2) == 1) {
					Bukkit.broadcastMessage(ChatColor
							.translateAlternateColorCodes('&',
									"&8&l» &b" + p.getName()
											+ " won... &aa Common Crate Key!"));
					Manager.getInstance().giveKey(p, KeyType.COMMON);
				}
			} else if (r.nextInt(5) == 1) {
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes(
						'&', "&8&l» &b" + p.getName()
								+ " won... &aKit Gummy!"));
				Bukkit.getServer().dispatchCommand(Bukkit.getServer().getConsoleSender(), "givekit " + p.getName() + " gummy");
				
				
			} else if (r.nextInt(5) == 2) {
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes(
						'&', "&8&l» &b" + p.getName()
								+ " won... &atheir very own pet doggy!"));
				Wolf wolf = (Wolf) p.getWorld().spawnEntity(p.getLocation(),
						EntityType.WOLF);
				wolf.setCustomNameVisible(true);
				wolf.setCustomName(ChatColor.BLUE + p.getName() + "'s "
						+ ChatColor.AQUA + "pet doggy");
				wolf.setTamed(true);
				wolf.setOwner(p);
				...
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes(
						...
						"&8&l» &b"
								+ p.getName()
								+ " won... &6a money party for all players online!"));
				for (Player online : Bukkit.getOnlinePlayers()) {
					online.playSound(p.getLocation(), Sound.FIREWORK_LAUNCH,
							1F, 1F);
					online.playSound(p.getLocation(),
			...
			if (r.nextInt(3) == 0) {
				...
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes(
						...
						"&8&l» &b"
								+ p.getName()
								+ " won... &6a Random Voucher!"));
				Manager.getInstance().giveVoucher(p, VoucherType.RANDOM);
			} else {
				Bukkit.broadcastMessage(ChatColor.translateAlternateColorCodes(
						'&', "&8&l» &b" + p.getName()
								+ " won... &a Haribo Kit!"));
				Bukkit.getServer().dispatchCommand(Bukkit.getServer().getConsoleSender(), "givekit " + p.getName() + " Haribo");
			}
			p.updateInventory();
		}
	}
			...
			if (lines[1] != null) {
				...
				if (lines[3].contains("$") && Utils.isInt(lines[3].replaceAll("[$]", "").trim())) {
					...
					if (Utils.onlyContainsLetters(lines[1])) {
						...
						if (p.getInventory().containsAtLeast(new ItemStack(Material.getMaterial(lines[1].replace(" ", "_").toUpperCase())), amount)) {
							
							Utils.removeItems(p.getInventory(), Material.getMaterial(lines[1].replace(" ", "_").toUpperCase()), 0, amount);
							p.updateInventory();
							
							mEconAPI.addMoney(p, cost);
							
							p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&8» &aYou sold &e" + amount + " " + lines[1].trim().replaceAll("_", " ") + "(s) &aand received &e$" + cost + "&a!"));
							
							return;
						} else p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&8» &cYou do not have enough items to sell!"));
					...
					if (Utils.isInt(lines[1])) {
						...
						if (p.getInventory().containsAtLeast(new ItemStack(Material.getMaterial(Integer.parseInt(lines[1]))), amount)) {
							
							String material = Material.getMaterial(Integer.parseInt(lines[1])).toString().replaceAll("_", " ").toLowerCase();
							String materialName = material.substring(0, 1).toUpperCase() + material.substring(1);
							
							Utils.removeItems(p.getInventory(), Material.getMaterial(Integer.parseInt(lines[1])), 0, amount);
							p.updateInventory();
							
							mEconAPI.addMoney(p, cost);
							
							p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&8» &aYou sold &e" + amount + " " + materialName + "(s) &aand received &e$" + cost + "&a!"));
					...
					if (Utils.matchesItemId(lines[1])) {
						...
						if (p.getInventory().containsAtLeast(item, amount)) {
							...
							String material = Material.getMaterial(Integer.parseInt(data[0])).toString().replaceAll("_", " ").toLowerCase();
							String materialName = material.substring(0, 1).toUpperCase() + material.substring(1);
							
							Utils.removeItems(p.getInventory(), Material.getMaterial(Integer.parseInt(data[0])), Integer.parseInt(data[1]), amount);
							p.updateInventory();
				...
				if (mEconAPI.removeMoney(p, cost)) {
						...
						try {
							...
							if (lines[1].contains("_")) {
								...
								for (int i = 0; i < args.length; i++) {
									...
									builder.append(args[i].substring(0, 1).toUpperCase() + args[i].substring(1).toLowerCase() + "_");
								...
								item = new ItemStack(Material.getMaterial(lines[1].toUpperCase().trim().replaceAll(" ", "_")), amount);
								p.getInventory().addItem(item);
								
								p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&8» &aYou bought &e" + amount + " " + materialName + "(s) &aand your account was debited &e$" + cost + "&a!"));
								...
								String materialName = material.substring(0, 1).toUpperCase() + material.substring(1).toLowerCase();
					...
					if (Utils.isInt(lines[1])) {
						...
						try {
							...
							p.sendMessage(ChatColor.translateAlternateColorCodes('&', "&8» &aYou bought &e" + amount + " " + materialName.replaceAll("_", " ") + "(s) &aand your account was debited &e$" + cost + "&a!"));
					...
					if (Utils.matchesItemId(lines[1])) {
						...
						try {
							...
							item = new ItemStack(Material.getMaterial(Integer.parseInt(data[0])), amount, Short.parseShort(data[1]));
							p.getInventory().addItem(item);
