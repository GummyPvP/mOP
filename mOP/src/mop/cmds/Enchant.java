		...
		if (cmd.getName().equalsIgnoreCase("enchant")) {
			...
			if (args.length == 2) {
				...
				if (p.getItemInHand() == null) {
					sender.sendMessage(
							ChatColor.translateAlternateColorCodes('&', "&8&l» &cYou must be holding a item!"));
					return true;
				}
				try {
					if (args[0].equalsIgnoreCase("fire")) {
						...
						p.getItemInHand().addUnsafeEnchantment(Enchantment.DAMAGE_ARTHROPODS,
						...
						p.getItemInHand().addUnsafeEnchantment(Enchantment.PROTECTION_ENVIRONMENTAL,
								Integer.parseInt(args[1]));
						p.updateInventory();
						return true;
					...
					sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
							"&8&l» &cYou cannot apply that enchantment to that item!"));
					return true;
				}
