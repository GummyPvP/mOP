...
public class CrateGUI {
	...
	public CrateGUI(Player p) {
		...
		inv = Bukkit.getServer().createInventory(p, 9 * 3,
				ChatColor.translateAlternateColorCodes('&', "&b&lGummyPvP Crates"));

		playerCrateAmounts();

		commonkey = ItemManager.createItem(Material.TRIPWIRE_HOOK, ChatColor.GREEN + "Common Crate Key",
				Arrays.asList(ChatColor.translateAlternateColorCodes('&',
						"&bYour inventory contains " + commonkeyamount + " Common Crate Key(s)!")));

		uncommonkey = ItemManager.createItem(Material.TRIPWIRE_HOOK, ChatColor.GOLD + "Uncommon Crate Key",
				Arrays.asList(ChatColor.translateAlternateColorCodes('&',
						"&bYour inventory contains " + uncommonkeyamount + " Uncommon Crate Key(s)!")));

		rarekey = ItemManager.createItem(Material.TRIPWIRE_HOOK, ChatColor.RED + "Rare Crate Key",
				Arrays.asList(ChatColor.translateAlternateColorCodes('&',
						"&bYour inventory contains " + rarekeyamount + " Rare Crate Key(s)!")));
		legendarykey = ItemManager.createItem(Material.TRIPWIRE_HOOK, ChatColor.DARK_PURPLE + "Legendary Crate Key",
				Arrays.asList(ChatColor.translateAlternateColorCodes('&',
						"&bYour inventory contains " + legendarykeyamount + " Legendary Crate Key(s)!")));
